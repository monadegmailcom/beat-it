...
class ArenaAllocator
{
  public:
    ...
    // thread-safe.
    template < typename T > void* allocate( size_t n = 1 ) // NOSONAR
    {
    ...
    // not thread safe.
    size_t allocated_blocks() const noexcept { return blocks.size(); }

    // not thread safe.
    size_t get_current_offset() const noexcept { return current_offset; }
...
class GenerationalArenaAllocator
{
  public:
    ...
    operator=( GenerationalArenaAllocator const& ) = delete;
