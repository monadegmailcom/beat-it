...
const State empty_state = {
    { ttt::empty_state, ttt::empty_state, ttt::empty_state, ttt::empty_state,
      ttt::empty_state, ttt::empty_state, ttt::empty_state, ttt::empty_state,
      ttt::empty_state },
...
Player::Player( Game const& game, double weight, unsigned depth, unsigned seed )
...
array< float, G > Player::serialize_state( Game const& game ) const
{
    ...
    // All input planes must have the same spatial dimensions for the Conv2d
    // layers. This provides global context to the local convolutional kernels,
    // a technique from the AlphaGo Zero paper.
    if ( current_player == PlayerIndex::Player1 )
        std::fill( plane4_player_indicator, plane4_player_indicator + 81,
                   1.0f );
    ...
    optional< uttt::Move >& move, PlayerIndex, uttt::State const& state )
{
    if ( !move ) // reset first move
        move = uttt::Move{ state.next_big_move != ttt::no_move
                               ? state.next_big_move
                               : ttt::Move{ 0 },
                           0 };
    else
    ...
    if ( state.next_big_move == ttt::no_move ) // free big move
    {
        for ( ; move->big_move < 9; ++move->big_move, move->small_move = 0 )
            if ( state.big_state[move->big_move] == GameResult::Undecided &&
                 next_move( move->small_move,
                            state.small_states[move->big_move] ) )
                return;
    }
    else if ( next_move(
                  // fixed big move
...
void append_valid_moves( uttt::State const& state, ttt::Move big_move,
...
{
    if ( move.big_move >= 9 || ( state.next_big_move != ttt::no_move &&
                                 move.big_move != state.next_big_move ) )
        throw std::invalid_argument( "invalid big move" );

    uttt::State new_state = state;
    new_state.small_states[move.big_move] =
        GameState< ttt::Move, ttt::State >::apply(
            move.small_move, player_index, state.small_states[move.big_move] );
    new_state.big_state[move.big_move] =
        GameState< ttt::Move, ttt::State >::result(
            player_index, new_state.small_states[move.big_move] );
    new_state.next_big_move =
        ( new_state.big_state[move.small_move] == GameResult::Undecided )
            ? move.small_move
            : ttt::no_move;
...
GameResult
GameState< uttt::Move, uttt::State >::result( PlayerIndex,
                                              uttt::State const& state )
{
    ...
    for ( const auto& win : ttt::wins )
    {
        ...
        if ( symbol != GameResult::Undecided && symbol != GameResult::Draw &&
             symbol == state.big_state[win[1]] &&
             symbol == state.big_state[win[2]] )
        {
            state.game_result_cache = ( symbol == GameResult::Player1Win )
    ...
    // check for undecided
    if ( ranges::any_of( state.big_state, []( GameResult symbol )
                         { return symbol == GameResult::Undecided; } ) )
    {
...
{
    os << "(" << (int)move.big_move << ',' << (int)move.small_move << ")";
    return os;
}
} // namespace uttt
