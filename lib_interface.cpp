...
enum class GameType : int32_t { TTT = 1, UTTT = 2 };

// Use C-style linkage to prevent C++ name mangling, making it callable from
// Python.
extern "C" {
evaluation::EvaluationStats
evaluate_models(int32_t game_type, const char *model1_data, uint32_t model1_len,
                const char *model2_data, uint32_t model2_len,
                libtorch::Hyperparameters const &hp, int32_t rounds,
                const char *save_path_c, const char *run_name_c, int32_t step) {
  ...
  if (game_type == static_cast<int32_t>(GameType::TTT)) {
    auto f =
        evaluation::evaluate<ttt::Move, ttt::State, ttt::alphazero::Player>;
    stats = f(m1, m2, hp, rounds, save_path, run_name, step, ttt::empty_state,
              seed, 50 * hp.simulations * sizeof(ttt::alphazero::Node));
  } else if (game_type == static_cast<int32_t>(GameType::UTTT)) {
    auto f =
        evaluation::evaluate<uttt::Move, uttt::State, uttt::alphazero::Player>;
    stats = f(m1, m2, hp, rounds, save_path, run_name, step, uttt::empty_state,
              seed, 50 * hp.simulations * sizeof(uttt::alphazero::Node));
  } else {
...
template <typename PlayerT> struct Session {
  ...
  using position_type =
      alphazero::training::Position<PlayerT::game_size, PlayerT::policy_size>;
  using player_type = PlayerT;
  using selfplay_type =
      alphazero::training::SelfPlay<typename game_type::move_type, state_type,
  ...
  Session(state_type const &initial_state,
    ...
    for (size_t i = 0; i < hp.parallel_games; ++i)
      allocators.push_back(
          make_unique<allocator_type>(hp.nodes_per_block * sizeof(node_type)));

    cout << "start " << thread_pool.size() << " parallel games" << endl;
    size_t i = 0;
    for (auto &future : thread_pool)
      future = async(&Session::worker, this, std::ref(*allocators[i++]));
  }
  ...
  void fetch_selfplay_data(DataPointers &data_pointers_out,
    ...
    for (size_t i = 0; i < number_of_positions; ++i) {
      ...
      ranges::copy(pos.game_state,
                   data_pointers_out.game_states + i * PlayerT::game_size);
      ranges::copy(pos.target_policy,
  ...
  void worker(allocator_type &allocator) {
    ...
    for (PlayerIndex player_index = PlayerIndex::Player1; !cleanup_requested;
         player_index = toggle(player_index)) {
      // Check for pause
      {
        unique_lock lock(pause_mutex);
        pause_cv.wait(lock, [this] { return !paused || cleanup_requested; });
      }
      if (cleanup_requested)
        break;
      ...
      player_type player(game_type(player_index, initial_state), ucb_params,
                         gameplay_params, g(), allocator, inference_service);

      positions.clear();
      selfplay_type selfplay(player, hp.dirichlet_alpha, hp.dirichlet_epsilon,
                             g, positions, root_node_entropy_stat,
                             informed_selection_stat);
      selfplay.run();
      {
...
// return number of created positions
uint32_t measure_selfplay_throughput(GameType game_type, const char *model_data,
                                     uint32_t model_data_len,
                                     libtorch::Hyperparameters const &hp,
                                     uint32_t number_of_positions) {
  ...
  if (game_type == GameType::TTT) {
    ...
    total_positions =
        uttt_session->measure_selfplay_throughput(number_of_positions);
  }
...
void set_model(void *session, GameType game_type, const char *model_data,
  ...
  if (game_type == GameType::TTT) {
    auto ttt_session = static_cast<ttt_session_type *>(session);
    ttt_session->set_model({model_data, model_data_len}, inference_batch_size,
                           inference_time);
    scoped_lock _(ttt_session->allocator_mutex);
    statistic_to_cpp_stat(ttt_session->allocator_stat, allocator_size);
  } else if (game_type == GameType::UTTT) {
    auto uttt_session = static_cast<uttt_session_type *>(session);
    uttt_session->set_model({model_data, model_data_len}, inference_batch_size,
                            inference_time);
...
void fetch_selfplay_data(void *session, GameType game_type,
  ...
  if (game_type == GameType::TTT) {
    ...
    statistic_to_cpp_stat(ses->inference_service.inference_time_stats(),
                          inference_time);
