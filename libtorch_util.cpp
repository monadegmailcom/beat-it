...
load_model( const char *model_path, torch::Device device )
{
    ...
    if ( !model_file )
        throw invalid_argument( "Failed to open model file: " +
                                string( model_path ) );
...
load_model( DataBuffer model_buffer, DataBuffer metadata_buffer,
            torch::Device device )
{
    return make_pair( load_model( model_buffer, device ),
                      Hyperparameters( string( metadata_buffer.data,
                                               metadata_buffer.len ) ) );
}
...
Hyperparameters::Hyperparameters( string const &metadata_json )
{
    ...
    dirichlet_alpha =
        get_required_value< float >( sp_config, "dirichlet_alpha" );
    dirichlet_epsilon =
        get_required_value< float >( sp_config, "dirichlet_epsilon" );
