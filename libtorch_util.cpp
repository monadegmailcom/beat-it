...
T get_required_value(const boost::json::object &obj, string const &key) {
  if (!obj.contains(key))
    throw invalid_argument("Required key not found in self_play_config: '" +
                           key + "'");
...
load_model(const char *model_path, torch::Device device) {
  ...
  if (!model_file)
    throw invalid_argument("Failed to open model file: " + string(model_path));
  ...
  string command =
      "unzip -p " + string(model_path) + " '*/extra/metadata.json' 2>/dev/null";
  FILE *pipe = popen(command.c_str(), "r");
  if (!pipe)
    throw system_error(errno, system_category(), "popen() for unzip failed");
  ...
  if (metadata_json.empty())
    throw invalid_argument("Failed to extract metadata.json from model file.");
...
unique_ptr<torch::jit::script::Module> load_model(DataBuffer model_buffer,
                                                  torch::Device device) {
  ...
  auto model = make_unique<torch::jit::script::Module>(
      torch::jit::load(model_data_stream, device));
  model->eval();
...
Hyperparameters::Hyperparameters(string const &metadata_json) {
  ...
  parallel_games =
      get_value_with_default<size_t>(sp_config, "parallel_games", 1);
  parallel_simulations =
      get_value_with_default<size_t>(sp_config, "parallel_simulations", 10);
  max_batch_size =
      get_value_with_default<size_t>(sp_config, "max_batch_size", 100);
  nodes_per_block = get_value_with_default<size_t>(sp_config, "nodes_per_block",
                                                   50 * simulations);
}
