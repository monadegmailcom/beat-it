...
// promise: model is set to eval mode
std::pair<std::unique_ptr<torch::jit::script::Module>, Hyperparameters>
load_model(DataBuffer model_buffer, DataBuffer metadata_buffer, torch::Device);
// promise: model is set to eval mode
std::unique_ptr<torch::jit::script::Module> load_model(DataBuffer model_buffer,
                                                       torch::Device);
...
public:
  ...
  InferenceService(std::unique_ptr<torch::jit::script::Module> &&model,
      ...
      : service_type(max_batch_size), device(device), model(std::move(model)) {
    ...
    cpu_input_tensor = torch::empty(
        {static_cast<long>(max_batch_size), static_cast<long>(G)}, cpu_options);

    auto gpu_options =
        torch::TensorOptions().device(device).dtype(torch::kFloat32);
    gpu_input_tensor = torch::empty(
        {static_cast<long>(max_batch_size), static_cast<long>(G)}, gpu_options);

    cpu_value_tensor =
        torch::empty({static_cast<long>(max_batch_size), 1}, cpu_options);
    cpu_policy_tensor = torch::empty(
        {static_cast<long>(max_batch_size), static_cast<long>(P)}, cpu_options);
  }

  // threadsafe replacement of model
  void update_model(std::unique_ptr<torch::jit::script::Module> &&new_model,
...
private:
  void inference(service_type::request_type request_batch[],
    ...
    for (size_t i = 0; i < batch_size; ++i) {
      std::copy_n(request_batch[i].state.data(), G, tensor_data_ptr + i * G);
    }
