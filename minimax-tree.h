...
struct Payload
{
    Payload( Payload const &payload ) : evaluation( payload.evaluation.load() )
    {
    }
...
class Player : public ::Player< MoveT >
{
  public:
    ...
    virtual double score( Game< MoveT, StateT > const & ) const { return 0; };
  ...
  private:
    ...
    double eval( node_type &node, unsigned depth, double alpha, double beta )
    {
        ...
        if ( GameResult result = node.get_game_result();
        ...
        prefix.splice( prefix.begin(), node.get_children(),
        ...
        prefix.sort(
            ...
            {
                return compare( a.get_payload().evaluation,
                                b.get_payload().evaluation );
            } );
    ...
    MoveT choose_move() override
    {
        ...
        // evaluation will benefit from better pruning from ordering of previous
        // step always start from level 0 because pruning my be different from
        // last time due to initialized alpha/beta start values
